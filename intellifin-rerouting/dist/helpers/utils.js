...
class Utils {
    static getVasProduct(url, payloadData, refrenceId) {
        if (url.includes('airtimevtu'))
            return { 'airtimevtu': 'airtimevtu', 'url': `${process.env.AIRTIME_URL}?phone=${payloadData['vasData'].phone}&amount=${payloadData['amount'] / 100}&service_type=${payloadData['vasData'].service_type}&plan=${payloadData['vasData'].plan}&agentId=${process.env.AGENT_ID}&agentReference=${refrenceId}` };
        if (url.includes('fundtransfer'))
            return 'fundtransfer';
        if (url.includes('data'))
            return { 'data': 'data', 'url': `${process.env.DATA_URL}?phone=${payloadData['vasData'].phone}&amount=${(payloadData['amount']) / 100}&service_type=${payloadData['vasData'].service_type}&datacode=${payloadData['vasData'].datacode}&agentId=${process.env.AGENT_ID}&agentReference=${refrenceId}` };
    ...
    static getVasLookUp(url, payloadData) {
        if (url.includes('airtimevtu'))
            return {
        ...
        if (url.includes('data/databundles'))
            return {
                'data': 'data',
                'method': "POST",
    ...
    static getTransProduct(url) {
        if (url.includes('purchase'))
            return '1';
        if (url.includes('cashadvance'))
            return '45';
        if (url.includes('refund'))
            return '5';
        if (url.includes('reversal'))
            return '4';
        if (url.includes('balance'))
            return '7';
        if (url.includes('preauth'))
            return '33';
        if (url.includes('salescompletion'))
            return '34';
        return false;
    }
    static getMaskPan(pan) {
    ...
    static fileDataLogger(terminalId, data) {
        ...
        if (enable != "true")
            return;
        let filesPath = path_1.default.join(`mw-logs/${(0, moment_1.default)().format("YYYY-MM-DD")}`, terminalId + `-logfile.log`);
        let pathDir = path_1.default.dirname(filesPath);
        if (fs_1.default.existsSync(pathDir) == false) {
